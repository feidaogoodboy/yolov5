...
def resblock_body(x, num_filters, num_blocks, expansion=0.5, shortcut=True, last=False, weight_decay=5e-4, name=""):
    ...
    x = C3(x, num_filters, num_blocks, shortcut=shortcut, expansion=expansion, weight_decay=weight_decay,
           name=name + '.1')
    if last:
...
def darknet_body(x, base_channels, base_depth, weight_decay=5e-4):
    ...
    x = DarknetConv2D_BN_SiLU(base_channels, (6, 6), strides=(2, 2), weight_decay=weight_decay,
                              name='backbone.stem.conv')(x)
    # 320, 320, 64 => 160, 160, 128
    ...
    feat3 = x
    return feat1, feat2, feat3  # （80， 80， 256），（40， 40，512），（20， 20， 1024）
