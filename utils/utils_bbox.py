...
def get_anchors_and_decode(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    # ------------------------------------------#
    grid_shape = K.shape(feats)[1:3]  # (20,20)
    # --------------------------------------------------------------------#
    #   获得各个特征点的坐标信息。生成的shape为(20, 20, num_anchors, 2)
    # --------------------------------------------------------------------#
    # grid_x :(20,) -> (1,20,1,1) -> (20, 20, 3, 1)
    grid_x = K.tile(K.reshape(K.arange(0, stop=grid_shape[1]), [1, -1, 1, 1]), [grid_shape[0], 1, num_anchors, 1])
    grid_y = K.tile(K.reshape(K.arange(0, stop=grid_shape[0]), [-1, 1, 1, 1]), [1, grid_shape[1], num_anchors, 1])
    grid = K.cast(K.concatenate([grid_x, grid_y]), K.dtype(feats))  # (20, 20, 3, 2)
    # ---------------------------------------------------------------#
    #   将先验框进行拓展，生成的shape为(20, 20, num_anchors, 2)
    # ---------------------------------------------------------------#
    anchors_tensor = K.reshape(K.constant(anchors), [1, 1, num_anchors, 2])  # (3,2) -> (1,1,3,2)
    anchors_tensor = K.tile(anchors_tensor, [grid_shape[0], grid_shape[1], 1, 1])  # (20,20,3,2),给每一个方格都给3个对应尺度的archor
    ...
    #   80代表的是种类的置信度
    # ---------------------------------------------------#
    # feats：(m,20,20,3,25) -> (bs,20,20,3,25)
    feats = K.reshape(feats, [-1, grid_shape[0], grid_shape[1], num_anchors, num_classes + 5])
    # ------------------------------------------#
    #   对先验框进行解码，并进行归一化
    # ------------------------------------------#
    # 根据预测的tx,ty,tw,th计算他们的真实中心坐标和宽高，并归一化
    # (m,20,20,3,2)
    ...
    #   获得预测框的置信度
    # ------------------------------------------#
    box_confidence = K.sigmoid(feats[..., 4:5])  # (m, 20, 20, 3, 1)
    box_class_probs = K.sigmoid(feats[..., 5:])  # (m, 20, 20, 3, 20)

    # ---------------------------------------------------------------------#
    #   在计算loss的时候返回grid, feats, box_xy, box_wh
    #   在预测的时候返回box_xy, box_wh, box_confidence, box_class_probs
    # ---------------------------------------------------------------------#
    if calc_loss == True:  # 训练
        # grid：(20, 20, 3, 2)，网格点坐标
        # feats：(bs,20,20,3,25) 原始预测信息
        # box_xy：(m,20,20,3,2) 根据预测的偏移量和网格坐标计算出来并归一化的预测框的真实中心点坐标
        # box_wh：(m,20,20,3,2) 根据预测的偏移量和网格坐标计算出来并归一化的预测框的真实宽高
        return grid, feats, box_xy, box_wh
    return box_xy, box_wh, box_confidence, box_class_probs  # 预测
